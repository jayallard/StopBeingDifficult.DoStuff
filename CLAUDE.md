...
3. `TaskFactory` builds the runnable `ITask`: for `WorkingDirectory`/`EnvironmentVariables`, `ParameterTemplate.Substitute` does `{ParamName}` → value substitution as before. For `Command`, parameters are **not** substituted into the script text — instead `TaskFactory` prepends one `$ParamName = '...'` assignment per parameter (value single-quote-escaped for PowerShell), wrapped in `# --- Parameters ---` / `# --- End Parameters ---` comment markers, before the task-authored script, so the script references `$ParamName` directly rather than embedding raw values inline.
